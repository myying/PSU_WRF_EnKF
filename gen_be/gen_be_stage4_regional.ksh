...
#      fi
      echo "Submitting job for variable $VARIABLE and vertical index $VINDEX"
      $SCRIPT_DIR/job_submit.sh 1 $JOB $HOSTPPN ./gen_be_stage4_regional.exe > gen_be_stage4_regional_${VARIABLE}_${VINDEX}.out 2>&1 &

      let JOB=$JOB+1
      let VINDEX=$VINDEX+1

      if [[ $JOB -eq $NUM_JOBS ]]; then
