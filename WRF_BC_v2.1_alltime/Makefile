...
make_rules:
	...
	if [ $$? = 0 ] ; then echo "Compiling for Compaq"				; \
		...
		echo "LIBS	= -L/usr/local/netcdf/lib -lnetcdf"	>> compiler_macros	; \
	...
	if [ $$? = 0 ] ; then echo "Compiling for IBM"						; \
		...
		echo "LIBS	= -L/usr/local/lib32/r4i4 -lnetcdf"	>> compiler_macros	; \
	...
	if [ $$? = 0 ] ; then echo "Compiling for Mac OS X"						; \
		...
		echo "LIBS	= -L/usr/local/netcdf-3.5.1-xlf/lib -lnetcdf"	>> compiler_macros	; \
	...
	if [ $$? = 0 ] ; then echo "Compiling for Linux"				; \
		...
		echo "LIBS	= -L$(NETCDF)/lib -lnetcdf -lnetcdf"	>> compiler_macros	; \
