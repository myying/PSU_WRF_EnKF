...
i_parent_start="1 "
j_parent_start="1 "
outfile=$WORK_DIR/fc/$DATE/wrfinput_d02_001
...
for n in `seq 1 $MAX_DOM`; do
  ...
  mv wrfinput_$dm wrfinput_${dm}_ndown
done
...
for n in 3; do
  ...
  ln -fs ../wrfinput_${dm}_ndown wrfndi_d02
  ...
  mv wrfinput_d02 ../wrfinput_${dm}
  cd ..
done

export run_minutes=$LBC_INTERVAL
export start_date=$DATE
...
next_date=`advance_time $start_date $run_minutes`
start_date=$next_date


###followed by restart runs with update sst
while [[ $next_date -lt $DATE_END ]]; do  #time loop
