...
krange = $(for k in ${KRANGE[*]}; do printf '%5.2f, ' `echo $k*${DX[$domain_id-1]}/${DX[1]} |bc -l`; done)
...
hroi_hurricane_PI = $(printf %.0f `echo $HROI_HURRICANE*$local_factor/$dx |bc -l`),
...
hroi_surface     = $(printf %.0f `echo $HROI_SFC*$local_factor/$dx |bc -l`),
vroi_surface     = $VROI,
/
...
hroi_sounding     = $(printf %.0f `echo ${HROI_SOUNDING:-$HROI_UPPER}*$local_factor/$dx |bc -l`),
...
hroi_profiler     = $(printf %.0f `echo ${HROI_PROFILER:-$HROI_UPPER}*$local_factor/$dx |bc -l`),
...
hroi_aircft     = $(printf %.0f `echo $HROI_UPPER*$local_factor/$dx |bc -l`),
vroi_aircft     = $VROI,
/
...
hroi_metar     = $(printf %.0f `echo $HROI_SFC*$local_factor/$dx |bc -l`),
vroi_metar     = $VROI,
/
...
hroi_sfcshp     = $(printf %.0f `echo $HROI_SFC*$local_factor/$dx |bc -l`),
vroi_sfcshp     = $VROI,
/
...
hroi_spssmi     = $(printf %.0f `echo $HROI_UPPER*$local_factor/$dx |bc -l`),
vroi_spssmi     = $VROI,
/
...
hroi_atovs     = $(printf %.0f `echo ${HROI_ATOVS:-$HROI_UPPER}*$local_factor/$dx |bc -l`),
vroi_atovs     = ${VROI_ATOVS:-$VROI},
/

&satwnd_obs
use_satwnd      = .$USE_GEOAMVOBS.,
datathin_satwnd = `expr ${THIN_SATWND:-0} + $thin_factor`,
hroi_satwnd     = $(printf %.0f `echo ${HROI_SATWND:-$HROI_UPPER}*$local_factor/$dx |bc -l`),
...
hroi_seawind     = $(printf %.0f `echo ${HROI_SEAWIND:-$HROI_UPPER}*$local_factor/$dx |bc -l`),
...
hroi_gpspw     = $(printf %.0f `echo $HROI_SFC*$local_factor/$dx |bc -l`),
vroi_gpspw     = $VROI,
/
...
hroi_radar     = $(printf %.0f `echo $HROI_RADAR*$local_factor/$dx |bc -l`),
