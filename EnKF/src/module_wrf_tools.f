...
!------------------------------------------------------
      subroutine tdew_to_q ( pres, td, q, td_error, q_error )

      implicit none

     real, intent(in)          :: pres        ! Observed pressure.
     real, intent(in)          :: td        ! Observed temperature.
     real, intent(in)          :: td_error  ! Dewpoint Temperature observation error.
     real, intent(out)         :: q        ! Observed specific humidity.
     real, intent(out)         :: q_error  ! q observation error.

     real                      :: e, e0    !vapor pressure.
     real                      :: pmb

      if ( abs(pres - missing) > 1. .and. abs(td - missing) > 1.) then
        pmb = pres/100.             !pa --> hPa

        e =6.112*17.67*243.5*td_error/((td-273.15+243.5)*(td-273.15+243.5))*&
             exp(17.67*(td-273.15)/(td-273.15+243.5))
        e0 = 6.112*exp(17.67*(td-273.15)/(td-273.15+243.5))

        q_error = 0.622*(pmb*e)/((pmb-e0)*(pmb-e0))*1000
        q = 0.622*e0/(pmb-e0)*1000
        if ( q < 0. ) q = 0.
